...
# ============================================================
# [改进4] 迭代验证与边界框精细化 —— 辅助函数
# ============================================================
def get_yes_no_confidence(model, processor, cropped_image, question):
    """
    获取模型对 yes/no 问题的 "yes" 置信度。
    复用 HiDe 现有的 model 和 processor, 不做额外加载。

    Args:
        model: MLLM 模型
        processor: 对应 processor
        cropped_image: PIL Image, 裁剪区域
        question: str, 验证问题 ("Is there a {entity} in this region?")
    ...
    # [优化4.3] 移除不必要的 torch.cuda.empty_cache()，避免强制 CUDA 同步开销
    del inputs, outputs
    return exp_yes / (exp_yes + exp_no)


# ============================================================
# [优化4.1] 批量验证 —— 将多个 (crop, question) 对合并为一次前向传播
# ============================================================
def batch_get_yes_no_confidence(model, processor, cropped_images, questions,
                                 max_batch_size=6):
    """
    批量获取模型对 yes/no 问题的 "yes" 置信度。
    将多个 (image, question) 对合并为 batch 前向传播，大幅减少前向传播次数。
    当总数超过 max_batch_size 时，自动拆分为多个子 batch 以避免 OOM。

    Args:
        model: MLLM 模型
        processor: 对应 processor
        cropped_images: List[PIL Image], 裁剪区域列表
        questions: List[str], 验证问题列表 (与 cropped_images 等长)
        max_batch_size: int, 单次前向传播的最大样本数 (根据 GPU 显存调整)

    Returns:
        List[float], "yes" 概率列表 (0~1)
    """
    ...
    # 按 max_batch_size 拆分子 batch, 避免 GPU OOM
    for batch_start in range(0, n, max_batch_size):
        batch_end = min(batch_start + max_batch_size, n)
        batch_crops = cropped_images[batch_start:batch_end]
        batch_questions = questions[batch_start:batch_end]
        m = len(batch_crops)

        # 为当前子 batch 中的每个 (image, question) 对构建消息
        all_texts = []
        all_image_inputs = []
        for cropped_image, question in zip(batch_crops, batch_questions):
            messages = [{
                "role": "user",
                "content": [
                    {"type": "image", "image": cropped_image},
        ...
        # 一次性批量处理当前子 batch
        inputs = processor(
            text=all_texts, images=all_image_inputs, videos=None,
            padding=True, return_tensors="pt"
        )
        inputs = inputs.to(model.device)
        ...
        # 找到每个 batch item 的最后一个非 padding token 位置
        seq_lengths = inputs.attention_mask.sum(dim=1) - 1  # shape [batch_size]

        for i in range(m):
            last_idx = seq_lengths[i].item()
            yes_logit = outputs.logits[i, last_idx, yes_id].item()
            no_logit = outputs.logits[i, last_idx, no_id].item()

            max_logit = max(yes_logit, no_logit)
            exp_yes = math.exp(yes_logit - max_logit)
            exp_no = math.exp(no_logit - max_logit)
            all_confidences.append(exp_yes / (exp_yes + exp_no))
...
def verify_and_refine_bboxes(model, processor, image, bboxes,
                              target_words, max_iterations=3,
                              conf_threshold=0.5, delta_0=0.2, gamma=0.8,
                              max_batch_size=6):#默认置信阈值0.8
    """
    [改进4] 批量版迭代验证与边界框精细化。

    核心改进:
    1. 先测试原始 bbox (无扩张), 通过则直接保留, 避免不必要的背景引入
    2. 仅对低置信度 bbox 进行迭代扩张验证
    3. fallback 时保留所有原始 bbox, 而非仅第一个

    Args:
        model: MLLM 模型
        processor: 对应 processor
        image: PIL Image 或 base64 str, 原始图像
        bboxes: List[List[float]], 归一化边界框 [[x0,y0,x1,y1], ...]
        target_words: List[str], 干净的语义目标实体名列表
        max_iterations: int, 最大扩张迭代次数
        conf_threshold: float, 接受阈值
        delta_0: float, 初始扩张比例
        gamma: float, 衰减因子

    Returns:
        List[List[float]], 验证/精细化后的边界框列表
    """
    if not bboxes or model is None:
        return bboxes
    ...
    # 收集所有有效实体词 (去重, 去空白)
    entities = list(set(w.strip() for w in target_words if w.strip()))
    if not entities:
        entities = ["the object"]

    num_bboxes = len(bboxes)
    best_confs = [0.0] * num_bboxes
    best_bboxes = list(bboxes)  # 初始化为原始 bbox
    active = [True] * num_bboxes

    # ================================================================
    # 第 0 步: 先验证所有原始 bbox (无扩张)
    # 如果原始 bbox 已经通过阈值, 直接保留, 不进入扩张循环
    # ================================================================
    batch_crops_orig = []
    batch_questions_orig = []
    batch_map_orig = []

    for bbox_idx in range(num_bboxes):
        nx0, ny0, nx1, ny1 = bboxes[bbox_idx]
        crop = pil_img.crop((
            int(nx0 * img_w), int(ny0 * img_h),
            int(nx1 * img_w), int(ny1 * img_h)
        ))
        for entity_idx, entity in enumerate(entities):
            verify_q = (f"Is there a {entity} clearly visible in this image? "
                        f"Answer yes or no.")
            batch_crops_orig.append(crop)
            batch_questions_orig.append(verify_q)
            batch_map_orig.append((bbox_idx, entity_idx))

    if batch_crops_orig:
        all_confs_orig = batch_get_yes_no_confidence(
            model, processor, batch_crops_orig, batch_questions_orig,
            max_batch_size=max_batch_size
        )
        # 按 bbox 分组取最高置信度
        bbox_orig_conf = {}
        for (bbox_idx, entity_idx), conf in zip(batch_map_orig, all_confs_orig):
            if bbox_idx not in bbox_orig_conf:
                bbox_orig_conf[bbox_idx] = 0.0
            if conf > bbox_orig_conf[bbox_idx]:
                bbox_orig_conf[bbox_idx] = conf

        for bbox_idx, conf in bbox_orig_conf.items():
            best_confs[bbox_idx] = conf
            if conf >= conf_threshold:
                # 原始 bbox 已通过, 不需要扩张
                active[bbox_idx] = False
    # ================================================================

    prev_confs = list(best_confs)  # 用于提前退出判断

    # ================================================================
    # 扩张迭代: 仅对未通过第 0 步的 bbox 进行
    # ================================================================
    for t in range(max_iterations):
        ...
        for bbox_idx in range(num_bboxes):
            if not active[bbox_idx]:
                continue
            nx0, ny0, nx1, ny1 = bboxes[bbox_idx]
            delta_x = delta_0 * (gamma ** t) * (nx1 - nx0)
            delta_y = delta_0 * (gamma ** t) * (ny1 - ny0)
            ...
            expanded_t[bbox_idx] = expanded

            crop = pil_img.crop((
                int(expanded[0] * img_w), int(expanded[1] * img_h),
                int(expanded[2] * img_w), int(expanded[3] * img_h)
            ))
            ...
            for entity_idx, entity in enumerate(entities):
                ...
                batch_questions.append(verify_q)
                batch_map.append((bbox_idx, entity_idx))

        if not batch_crops:
            break

        all_confs = batch_get_yes_no_confidence(
            model, processor, batch_crops, batch_questions,
            max_batch_size=max_batch_size
        )

        bbox_best_conf = {}
        for (bbox_idx, entity_idx), conf in zip(batch_map, all_confs):
            if bbox_idx not in bbox_best_conf:
                bbox_best_conf[bbox_idx] = 0.0
            if conf > bbox_best_conf[bbox_idx]:
                bbox_best_conf[bbox_idx] = conf

        for bbox_idx, conf in bbox_best_conf.items():
            if conf > best_confs[bbox_idx]:
                best_confs[bbox_idx] = conf
                best_bboxes[bbox_idx] = expanded_t[bbox_idx]

            if conf >= conf_threshold:
                active[bbox_idx] = False
            if t > 0 and conf <= prev_confs[bbox_idx] - 0.05:
                active[bbox_idx] = False

            prev_confs[bbox_idx] = conf
    # ================================================================

    # 筛选: 保留置信度 >= threshold * 0.5 的 bbox
    refined_bboxes = []
    for bbox_idx in range(num_bboxes):
        if best_confs[bbox_idx] >= conf_threshold * 0.5:
            refined_bboxes.append(best_bboxes[bbox_idx])

    # [修复] fallback: 如果所有框都被过滤, 保留所有原始 bbox (而非仅第一个)
    if not refined_bboxes and bboxes:
        refined_bboxes = list(bboxes)
...
def from_img_and_att_get_cropbox(inputs,attention, dicts, img_url, img_start, img_end,sig,thre,
    ...
    for s in sig:
        for t in thre:
            ...
            # ============================================================
            # [改进4] 迭代验证与边界框精细化
            # 在 LPD 之前对边界框进行置信度验证和自适应扩张
            # 启用: ENABLE_ITERATIVE_REFINEMENT=True 且 model/processor 已传入
            # 禁用: ENABLE_ITERATIVE_REFINEMENT=False 则直接跳回原版行为
            # ============================================================
            if ENABLE_ITERATIVE_REFINEMENT and model is not None:
                # [修复] 从 prompt 文本中提取干净的实体名,
                # 而非使用 words_lines 的累积文本碎片
                prompt_text = ""
                for i in range(start_k, end_k):
                    token_idx = inputs['input_ids'][0][i].cpu().item()
                    if token_idx < 151643:
                        prompt_text += dicts[token_idx]

                prefix = "Search the following entities in the images:"
                if prefix in prompt_text:
                    entity_str = prompt_text.split(prefix)[-1].strip()
                    clean_entities = [e.strip() for e in entity_str.split(",") if e.strip()]
                else:
                    # 回退: 无法解析时用 words_lines (保留原行为)
                    clean_entities = [w for w in words_lines.values() if w.strip()]
                if not clean_entities:
                    clean_entities = ["the object"]

                for imgidx in list(bounding_boxes.keys()):
                    refined = verify_and_refine_bboxes(
                        model, processor, img_url[imgidx],
                        bounding_boxes[imgidx], clean_entities,
                        max_iterations=3, conf_threshold=0.5
                    )
                    if refined:
                        bounding_boxes[imgidx] = refined
            # ============================================================
